...
function FeatureCard({ icon, title, desc }) {
  return (
    <div 
      className="w-full max-w-[260px] h-[285px] rounded-[15px] shadow-[-1px_7px_10px_rgba(0,0,0,0.25)] flex flex-col items-center justify-start pt-6 transition-transform hover:-translate-y-2 cursor-pointer relative shrink-0 mx-auto"
      style={{ background: 'conic-gradient(from 32.94deg at 58.08% 31.4%, #426A72 0deg, #33505D 219.81deg, #6F8E93 360deg)' }}
    >
      <img src={icon} alt={title} className={`w-[130px] h-[130px] object-contain drop-shadow-[5px_5px_10px_rgba(0,0,0,0.25)] mb-4 ${title === '무한한 건설' ? '-rotate-[5.68deg]' : ''}`} />
      <h3 className="text-white text-[32px] font-semibold drop-shadow-[0px_2px_2px_rgba(0,0,0,0.25)] mb-2">{title}</h3>
      <p className="text-[#D8D8D8] text-[15px] font-medium leading-tight text-center px-4 drop-shadow-[0px_2px_2px_rgba(0,0,0,0.25)]">{desc}</p>
    </div>
  );
}
...
export default function Content() {  
  ...
  return (  
    <section className="w-full h-auto xl:h-[386px] bg-gradient-to-b from-[#194D56] to-[#102A3E] relative z-10 mt-0 xl:mt-[-450px] pb-16 xl:pb-0">  
      <div className="max-w-[1440px] h-full mx-auto px-5 lg:px-10 flex flex-col xl:flex-row items-center justify-between gap-12 xl:gap-0 pt-10 xl:pt-0">
        <div className="flex flex-col sm:flex-row flex-wrap xl:flex-nowrap gap-6 lg:gap-8 justify-center w-full xl:w-auto">  
        ...
        {/* 오른쪽: 최신 업데이트 배너 영역 */}  
        <div className="flex flex-col gap-4 flex-shrink-0 relative w-full sm:w-[382px] items-center xl:items-start">
          ...
          {/* 마우스 및 터치 이벤트 */}
          <div 
            className="w-full max-w-[382px] h-[290px] overflow-hidden relative cursor-grab active:cursor-grabbing select-none"
            onMouseDown={handleDragStart} onMouseUp={handleDragEnd} 
            onMouseLeave={(e) => { if(e.buttons === 1) handleDragEnd(e) }} 
            onTouchStart={handleDragStart} onTouchEnd={handleDragEnd}
          >
            <div className="flex transition-transform duration-500 ease-out w-full" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
              {updateData.map((data) => (
                <a key={data.id} href={data.link} onClick={handleClick} draggable="false" className="w-full flex-shrink-0 block relative group/item select-none">  
